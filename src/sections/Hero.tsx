...
function GlitchName({ text }: { text: string }) {
  ...
  return (
    <div className="relative inline-block">
      <span className="font-mono text-4xl sm:text-5xl md:text-6xl font-bold text-heading leading-tight select-none">
        {text}
      </span>
      {active && (
        <>
          <span
            className="absolute inset-0 font-mono text-4xl sm:text-5xl md:text-6xl font-bold leading-tight text-red-400 opacity-80 pointer-events-none"
            style={{ animation: 'glitch-1 0.35s steps(1) forwards' }}
          >
            {text}
          </span>
          <span
            className="absolute inset-0 font-mono text-4xl sm:text-5xl md:text-6xl font-bold leading-tight text-cyan-400 opacity-80 pointer-events-none"
            style={{ animation: 'glitch-2 0.35s steps(1) forwards' }}
          >
            {text}
          </span>
        </>
...
/* ── Terminal data rows ─────────────────────────────── */
const rows = [
  { key: 'OPERATOR', value: null },          // name rendered separately
  { key: 'ROLE',     value: profile.title },
  { key: 'SCHOOL',   value: profile.education.schoolFull },
  { key: 'CLASS',    value: profile.education.period },
...
export default function Hero() {
  return (
    <section
      id="hero"
      className="relative min-h-screen grid-bg flex flex-col justify-center pt-20 pb-16 px-6 overflow-hidden"
    >
      ...
      <div className="relative max-w-5xl mx-auto w-full flex flex-col lg:flex-row items-center gap-10 lg:gap-16">
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-3 bg-surface2 border-b border-border">
              ...
              <span className="ml-3 font-mono text-xs text-muted">
                operator@sk-portfolio <span className="text-accent">~</span>/profile
              </span>
            </div>
            ...
            <div className="bg-bg/80 backdrop-blur-sm px-6 py-6 space-y-1">
              ...
              >
                ...
                <span className="text-heading">./identify</span>{' '}
                <span className="text-cyan">--target</span>{' '}
                <span className="text-muted">seif_koubaa --verbose</span>
              </motion.p>
              ...
              <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-2">
                {rows.map(({ key, value }) => (
                  <motion.div key={key} variants={rowAnim} className="flex items-baseline gap-3 font-mono text-sm">
                    <span className="text-muted w-20 shrink-0 text-xs">{key}</span>
              ...
              {/* Blinking cursor */}
              <motion.p
                className="font-mono text-sm text-muted mt-5"
          ...
          >
            {[
              ...
              { href: `mailto:${profile.email}`, label: 'Email',    icon: <MailIcon />,     style: 'hover:border-gold hover:text-gold' },
            ...
            <a href="/resume.pdf" download="Seif_Koubaa_Resume.pdf"
              className="inline-flex items-center gap-2 px-4 py-2 bg-accent/10 border border-accent/40 text-accent text-sm font-mono rounded hover:bg-accent/20 transition-all duration-200"
            >
              <DownloadIcon /> Resume
            </a>
          </motion.div>
        ...
        >
          ...
          <div className="relative p-3">
            ...
            {/* Floating hex labels */}
            <span className="absolute -top-1 -right-1 font-mono text-[10px] text-accent/60 bg-bg border border-accent/20 px-1.5 py-0.5 rounded">
              ID:SK_2026
            </span>
            <span className="absolute -bottom-1 -left-1 font-mono text-[10px] text-muted bg-bg border border-border px-1.5 py-0.5 rounded">
              0x4B4F55424141
            </span>
          </div>
